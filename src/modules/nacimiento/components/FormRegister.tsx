...
export const FormRegister = () => {
  ...
  const { startRegisterBirthDocument, startShowBirthMessage } =
    useBirthDocsStore();
  ...
  const [personsToAdd, setPersonsToAdd] = useState({
    principal_person: PersonObject,
    birth_mother: PersonObject,
    birth_father: PersonObject,
  });

  const handleSubmit = async (values: any) => {
    if (
      personsToAdd.principal_person._id === '' ||
      personsToAdd.birth_father._id === '' ||
      personsToAdd.birth_mother._id === ''
    ) {
      setErrorSearch({
      ...
      if (success) {
        ...
        toast.error('Ocurrio un error al momento de registrar', {
          transition: Bounce,
        });
      }
    }
          ...
          <Form>
            <div className='flex flex-col sm:flex-row w-full gap-x-8 xl:gap-x-6 2xl:gap-x-16'>
              <div className='flex flex-col w-full sm:w-1/2'>
                ...
                <SearchPerson
                  inputText=''
                  error={errorSearch.errorPerson}
                  placeHolder='Buscar persona...'
                  getPerson={(person: PersonOptions) =>
                    setPersonsToAdd({
                      ...personsToAdd,
              ...
              <div className='w-full sm:w-1/2 flex flex-col mt-3 sm:mt-0'>
                ...
                <SearchPerson
                  inputText=''
                  error={errorSearch.errorFather}
                  placeHolder='Buscar al padre...'
                  getPerson={(person: PersonOptions) =>
                    setPersonsToAdd({ ...personsToAdd, birth_father: person })
                  }
                />
            ...
            <div className='mt-1 sm:mt-3 flex flex-col sm:flex-row w-full gap-x-8 xl:gap-x-6 2xl:gap-x-16'>
              <div className='flex flex-col w-full sm:w-1/2 mt-3 sm:mt-0'>
                ...
                <SearchPerson
                  inputText=''
                  error={errorSearch.errorMother}
                  placeHolder='Buscar a la madre...'
                  getPerson={(person: PersonOptions) =>
                    setPersonsToAdd({ ...personsToAdd, birth_mother: person })
                  }
                />
              ...
              <div className='w-full sm:w-1/2 flex flex-col mt-3 sm:mt-0'>
                ...
                <Field
                  ...
                  className={`input_field ${
                    !errors.birth_book || !touched.birth_book
                      ? 'border-gray-400'
                ...
                <ErrorMessage
                  name='birth_book'
                  component={() => <MessageAlert message={errors.birth_book} />}
                />
              </div>
            </div>
            ...
            <div className='mt-1 sm:mt-3 flex flex-col w-full'>
              <div className='flex flex-col w-full sm:w-1/2 pe-0 sm:pe-4 xl:pe-3 2xl:pe-8 mt-3 sm:mt-0'>
                <label htmlFor='birth_date' className='font-semibold text-sm '>
                  Fecha de nacimiento:
                </label>
                <Field
                  type='date'
                ...
                <ErrorMessage
                  name='birth_date'
                  component={() => <MessageAlert message={errors.birth_date} />}
                />
              </div>
            </div>
            ...
            <div className='flex flex-row mt-10 sm:mt-16 w-full justify-center gap-x-12 text-white font-semibold'>
              <button
                type='submit'
                className='flex items-center gap-x-2 bg-green-700 px-7 py-2 rounded-lg shadow-lg shadow-green-950 hover:shadow-md hover:shadow-green-900 duration-300'
              >
              ...
              <button
                type='reset'
                className='flex items-center gap-x-2 bg-red-700 px-7 py-2 rounded-lg shadow-lg shadow-red-950 hover:shadow-md hover:shadow-red-900 duration-300'
              >
